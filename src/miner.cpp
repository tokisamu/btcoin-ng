...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewKeyBlock(const CScript& scriptPubKeyIn,std::string publicKey)
{
    int64_t nTimeStart = GetTimeMicros();
    ...
    int nPackagesSelected = 0;
    int nDescendantsUpdated = 0;
    //bitcoin-ng key block don't include transactions
    //addPackageTxs(nPackagesSelected, nDescendantsUpdated);
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewMicroBlock(const CScript& scriptPubKeyIn,std::string publicKey,std::string privateKey)
{
    int64_t nTimeStart = GetTimeMicros();
    ...
    LogPrintf("CreateNewMicroBlock(): block weight: %u txs: %u fees: %ld sigops %d\n", GetBlockWeight(*pblock), nBlockTx, nFees, nBlockSigOpsCost);
